# /test_120_second_radio_launch.py

"""
Test the enhanced 120-second planet launch sequence with radio chatter
"""
...
def test_120_second_radio_launch():
    """Test the 120-second planet launch sequence with radio chatter"""
    ...
    try:
        ...
        dialog.launch_approved.connect(on_launch_complete)
        
        # Monitor when launch sequence actually starts
        def check_launch_started():
            if hasattr(dialog, 'launch_phase') and dialog.launch_phase == "launching" and not sequence_started:
                start_timer.start()
                print(f"🚀 Enhanced launch sequence started with radio chatter at: {time.strftime('%H:%M:%S')}")
        ...
        print(f"🌍 Testing 120-second enhanced launch from {planet_name}")
        print("📡 Features: Launch control communication + Radio chatter during orbital insertion")
        print("🚀 Enhanced atmospheric exit with pilot ↔ ground control dialogue")
        print("⏱️  Sequence should take exactly 120 seconds (2 minutes)")
        print("🎭 Multiple voices: Ship Computer, Pilot, Launch Control")
        print()
        
        dialog.show()
        
        # Auto-accept after launch completes (for testing)
        def auto_complete():
            if hasattr(dialog, 'launch_phase') and dialog.launch_phase == "complete":
                print("\n✅ Auto-accepting completed launch dialog")
                dialog.accept()
            else:
