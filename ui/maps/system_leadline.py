# /ui/maps/system_leadline.py

"""System Map Lead Line Overlay
...
class SystemLeaderLineController(QObject):
    ...
    2) Our overlay is parented to the System viewport.
        ...
        try:
            ...
            # Must be on the System tab
            try:
                if tw.currentWidget() is not sol:
                    return False
            except Exception:
                return False

            # And our overlay must be attached to the System viewport
            sol_vp = self._widget_viewport(sol)
            return self._overlay_src is sol_vp
        except Exception:
            return False
    ...
    def _is_system_tab(self) -> bool:
        ...
        try:
            ...
            idx_system = tw.indexOf(sw)
            return idx_system >= 0 and idx_current == idx_system
        except Exception:
            return False
    ...
    def _ensure_tick_running(self) -> None:
        if self._is_system_tab() and self._line_active and (self._overlay is not None) and (self._overlay_src is not None):
    ...
    def _maybe_stop_tick(self) -> None:
        if self._tick.isActive() and (not self._line_active or self._overlay is None or not self._is_system_tab()):
            self._tick.stop()
    ...
    def _on_tick(self) -> None:
        ...
        # hard guard: only react if the cursor is actually in *this* panel's viewport
        inside = getattr(self._panel, "cursor_inside_viewport", None)
        if callable(inside) and not inside():
            self._line_active = False
            if self._overlay:
